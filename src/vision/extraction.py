...
Grid inputs accepted everywhere:
- ``None`` → uses ``DEFAULT_GRID`` (axis-aligned dict)
- A plain dict with ``origin_x/spacing_x/…`` keys
- A dict with a ``"corners"`` sub-dict → reconstructed as ``PlateGrid4``
- A :class:`~src.plate_geometry.PlateGrid` (axis-aligned dataclass)
- A :class:`~src.plate_geometry.PlateGrid4` (bilinear 4-corner model)
...
# ======================================================================
# Grid normalization — accepts dict, PlateGrid, or PlateGrid4
# ======================================================================

def _resolve_grid(grid):
    """Normalize any grid input into a usable object.

    Returns one of:
    - A :class:`PlateGrid4` (if 4 corners are available)
    - A :class:`PlateGrid` (axis-aligned dataclass)
    - A plain dict (legacy fallback)

    Dicts with a ``"corners"`` key are upgraded to :class:`PlateGrid4`.
    """
    if grid is None:
        return DEFAULT_GRID

    # Already a dataclass — use as-is
    from src.vision.geometry import PlateGrid, PlateGrid4
    if isinstance(grid, (PlateGrid, PlateGrid4)):
        return grid

    # Dict with corners → reconstruct PlateGrid4
...
def _get_roi_radius(grid) -> int:
    """Extract the integer ROI radius from any grid representation."""
    from src.vision.geometry import PlateGrid, PlateGrid4
    if isinstance(grid, (PlateGrid, PlateGrid4)):
        return int(round(grid.roi_radius))
    return int(grid["roi_radius"])
...
def get_well_center(
    ...
    Args:
        ...
        grid: Grid parameters (dict, PlateGrid, or PlateGrid4).
            Uses DEFAULT_GRID if None.

    Returns:
        (cx, cy) pixel coordinates.
    """
    g = _resolve_grid(grid)

    from src.vision.geometry import PlateGrid, PlateGrid4
    if isinstance(g, (PlateGrid, PlateGrid4)):
        from src.vision.geometry import get_well_center as _pg_get_well_center
        return _pg_get_well_center(row, col, g)

    # Plain dict path (legacy)
